...
func TestFullAnalysisFlow(t *testing.T) {
	...
	step4Result := step4_xiji.AnalyzeXiJi(&chart, step2Result, step3Result)
	t.Log("=== 第四步：喜忌用神分析 ===")
	t.Logf("  用神五行: %v", step4Result.YongShenWuXing)
	t.Logf("  用神十神: %v", step4Result.YongShenShiShen)
	t.Logf("  喜神五行: %v", step4Result.XiShenWuXing)
	t.Logf("  喜神十神: %v", step4Result.XiShenShiShen)
	t.Logf("  忌神五行: %v", step4Result.JiShenWuXing)
	t.Logf("  忌神十神: %v", step4Result.JiShenShiShen)
	t.Log("")
